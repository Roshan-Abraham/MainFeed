...
def vectorization(df, columns):
    ...
    if column_name not in ['Personality Traits', 'Interests']:
        return df
    
    if column_name in ['Personality Traits', 'Interests']:
...
def PredCluster(user_data):
    ...
    model = joblib.load(r"DateMainFeed.joblib")
    ...
    likely_users = new_list[[user_n]].sort_values(by=[user_n],axis=0, ascending=False)[1:50]
